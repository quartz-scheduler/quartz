 ...
 * @author Zemian Deng
 */
class QuartzDatabaseCronTriggerTest extends QuartzDatabaseTestSupport {
    @Test
    void testCronRepeatCount() throws Exception {
        ...
        MatcherAssert.assertThat(scheduledTimes, hasSize(greaterThanOrEqualTo(3)));
        
        Long[] times = scheduledTimes.toArray(new Long[scheduledTimes.size()]);
        long baseline = times[0];
        MatcherAssert.assertThat(baseline % 1000, is(0L));
        for (int i = 1; i < times.length; i++) {
            MatcherAssert.assertThat(times[i], is(baseline + TimeUnit.SECONDS.toMillis(i)));
        }
    }
}
