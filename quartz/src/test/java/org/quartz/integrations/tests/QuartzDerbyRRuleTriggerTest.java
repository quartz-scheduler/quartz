...
/**
 * An integration test for the Quartz JDBC (DERBY) scheduler with an
 * {@link RRuleTrigger}. This exercises the RRULE persistence delegate and the
 * {@code QRTZ_RRULE_TRIGGERS} DDL end to end.
 */
public class QuartzDerbyRRuleTriggerTest extends QuartzDerbyTestSupport {
    @Test
    void testRRuleRepeatCount() throws Exception {
        RRuleTrigger trigger = TriggerBuilder.newTrigger()
                .withIdentity("test")
                .withSchedule(RRuleScheduleBuilder.rruleSchedule("FREQ=SECONDLY"))
                .build();
